"""
Telegram-бот с RAG-функциональностью на основе Yandex GPT API.

Этот бот умеет:
- Отвечать на вопросы с использованием базы знаний (RAG)
- Обрабатывать изображения и извлекать текст (Vision - заглушка)
...
🖼 Обработка изображений и извлечение текста (в разработке)
...
/clear - Очистить историю разговора
Удаляет сохраненную историю сообщений для начала новой темы.

<b>4️⃣ Технические детали</b>
🔹 Модель чата: Yandex GPT Lite
🔹 Эмбеддинги: Yandex text-search-doc
🔹 Векторная БД: FAISS
...
• Провайдер: {stats.get('api_provider', 'Yandex Cloud')}
...
async def main():
    ...
    """
    logger.info("=" * 50)
    logger.info("Запуск Telegram-бота с RAG (Yandex GPT)")
    logger.info("=" * 50)

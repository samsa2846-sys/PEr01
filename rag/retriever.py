...
Retriever (извлекатель) - компонент RAG-системы, который находит
наиболее релевантные документы по запросу пользователя.
"""
...
class DocumentRetriever:
    ...
    def __init__(self, embedder: YandexEmbedder, vectorstore: FAISSVectorStore):
        """
    ...
    def retrieve_context(self, query: str, top_k: int = TOP_K_RESULTS, 
        ...
        Эта функция удобна для прямого использования в RAG-пайплайне,
        так как возвращает готовый контекст для языковой модели.
    ...
    def get_relevant_sources(self, query: str, top_k: int = TOP_K_RESULTS) -> List[str]:
        ...
        Полезно для отображения пользователю, откуда была взята информация.
